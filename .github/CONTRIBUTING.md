...
Thanks for using `@cyanheads/paleobiology-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

Open one from the [**Issues**](https://github.com/cyanheads/paleobiology-mcp-server/issues) tab and pick the **Bug Report** or **Feature Request** form. Both are structured, and filling in the fields is what makes an issue actionable. Anything that fits neither can be a plain issue — a half-formed idea in your own words is fine.

<!-- This project takes contributions as issues. Do not add a "pull requests are welcome" line. -->

## Server bug or framework bug?

`@cyanheads/paleobiology-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:

- **This repo** — a tool returns wrong data, a Paleobiology Database call fails, a schema doesn't match reality, a description misleads the model.
...
Don't open a public issue for a vulnerability. Report it privately — GitHub's **Security** tab → **Report a vulnerability**, or email the maintainer. See [SECURITY.md](./SECURITY.md).
